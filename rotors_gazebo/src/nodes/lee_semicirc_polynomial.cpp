...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, "lee_semicirc_polynomial");
  ros::NodeHandle nh;
  ...
  ROS_INFO("Start lee_semicirc_polynomial.");

  double xd, yd, zd;
  double x_b1, y_b1, z_b1;

  ROS_INFO("Started lee_semicirc_polynomial.");
  ...
  // Wait for 3 seconds to let the Gazebo GUI show up.
  if (ros::Time::now().toSec() < 5*START_SIMULATION_TIME){
    ros::Duration(START_SIMULATION_TIME).sleep();
  }
